...
from others.ptdec_eva.ptdec.utils import target_distribution, cluster_accuracy
...
def train(dataset: torch.utils.data.Dataset,
    ...
    ##TODO: we need to make fixes here


    predicted = kmeans.fit_predict(torch.cat(features).numpy().astype(np.float32))
    predicted_previous = torch.tensor(np.copy(predicted), dtype=torch.long).cpu()
    #_, accuracy = cluster_accuracy(predicted, actual.cpu().numpy()) ## you shouldn't have access, I guess it's just a sanity check
    accuracy = -1 ##TODO: We will just put a place holder for accuracy
    ...
    for epoch in range(epochs):
        ...
        assert(predicted.get_device() == -1)
        assert(predicted_previous.get_device() == -1)
