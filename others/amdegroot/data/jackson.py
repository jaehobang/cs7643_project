...
JACKSON_CLASSES = (  # always index 0
...
class JacksonAnnotationTransform(object):
    ...
    def __call__(self, boxes, labels, width, height):
        ...
        for i, box in enumerate(boxes):
            ...
            label_num = JACKSON_CLASSES.index(labels[i])
            assert(label_num >= 0 and label_num < len(JACKSON_CLASSES))
...
class JacksonDetection(data.Dataset):
    ...
    def __init__(self, transform=None,
                 target_transform=JacksonAnnotationTransform(), dataset_name='UA-DETRAC', eval = False):
