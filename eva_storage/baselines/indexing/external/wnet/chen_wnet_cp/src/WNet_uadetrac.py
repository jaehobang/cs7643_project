...
def tf_flags():
    ...
    tf.flags.DEFINE_integer("image_size", "256", "batch size for training")
    tf.flags.DEFINE_integer('max_iteration', "50000", "max iterations")
    tf.flags.DEFINE_integer('decay_steps', "10000", "max iterations")
    tf.flags.DEFINE_integer('num_class', "4", "number of classes for segmentation")
    tf.flags.DEFINE_integer('num_layers', "9", "number of layers of UNet")
    tf.flags.DEFINE_string("cmap", "viridis", "color map for segmentation")
    tf.flags.DEFINE_string("logs_dir", "WNet_uadetrac_logs/", "path to logs directory")
...
if __name__ == '__main__':
    ...
    if flags.mode == "test":
        ...
        train_dataset_reader, validation_dataset_reader = create_uadetrac()
