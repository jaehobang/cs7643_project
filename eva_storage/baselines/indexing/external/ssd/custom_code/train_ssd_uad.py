...
### I need to debug a lot of things,
### There are multiple problems that will be outlined here
"""
1. Confidence is over 1? Shouldn't there be a cap on this value
2. Boxes give negative numbers, regression loss is way to small compared to classification loss
...
def train(loader, net, criterion, optimizer, device, logging, debug_steps=100, epoch=-1):
    ...
    for i, data in enumerate(loader):
        ...
        loss = regression_loss + classification_loss ## we might want to amplify the regression loss -> amplified by 300
        loss.backward()
        optimizer.step()
...
if __name__ == "__main__":
    ...
    num_workers = 4 ##4
    num_epochs = 200
    checkpoint_folder = os.path.join(home_dir, 'eva_storage/baselines/indexing/external/ssd/', 'models/tmp')
    lr = 1e-3
    momentum = 0.9
    weight_decay = 5e-4
    validation_epochs = 5
    ...
    dataset_train = UADataset_lite(transform=train_transform, target_transform=target_transform)
    dataset_train.set_images(images_train)
    dataset_train.set_labels(labels_train)
    dataset_train.set_boxes(boxes_train)

    dataset_val = UADataset_lite(transform=train_transform, target_transform=target_transform)
    ...
    for epoch in range(last_epoch + 1, num_epochs):
        ...
        if epoch % validation_epochs == 0 or epoch == num_epochs - 1:
            ...
            val_classification_loss = running_classification_loss / num
            logger.info(f"epoch: {epoch}")
            logger.info("  Validation Loss: {v:.4f}".format(v=val_loss))
            logger.info("  Validatiion Regression Loss: {v:.4f}".format(v=val_regression_loss))
            logger.info("  Validation Classification Loss: {v:.4f}".format(v=val_classification_loss))

            checkpoint_file_name = "vgg16-ssd:epoch" + str(epoch)
            model_path = os.path.join(checkpoint_folder, checkpoint_file_name)
            net.save(model_path)

    logger.info(f"Total time to train...{time.perf_counter() - st} seconds")
