...
1. does different downsampling methods result in significant difference in accuracy??
...
from eva_storage.featureExtractionMethods import DownSampleMeanMethod, DownSampleLanczosMethod, DownSampleSkippingMethod
...
class Writer:
    ...
    def write(method_name, dataset_name, benchmark_results):
        write_directory = os.path.join('/nethome/jbang36/eva_jaeho/data/benchmark_results/downsample_method', dataset_name, method_name+'.txt')
...
def perform_benchmark_jvc_pp(images, gt_labels, pp_predictions, dataset_name, query_name, number_of_samples,
                             downsample_method = DownSampleMeanMethod(), vector_size = 100):
    preprocessor = Preprocessor(vector_size = vector_size, downsample_method = downsample_method)
    ...
    Writer.write(f'jvc_pp_{number_of_samples}_{downsample_method.name}', dataset_name, benchmark_results)


def perform_benchmark_hierarchy_pp(images, gt_labels, pp_predictions, dataset_name, query_name,
                                   number_of_samples, downsample_method = DownSampleMeanMethod(), vector_size = 100):
    preprocessor = Preprocessor(vector_size = vector_size, downsample_method = downsample_method)
    ...
    Writer.write(f'hierarchy_pp_{number_of_samples}_{downsample_method.name}', dataset_name, benchmark_results)
...
def perform_benchmarks(dataset, ssd_predictions, pp_predictions, label_name, query_name, number_of_samples):
    ...
    vector_size = [100]
    #downsample_methods = [DownSampleSkippingMethod(), DownSampleLanczosMethod(), DownSampleMeanMethod()]
    ## we've done DownSampleSkippingMethod, we need to try others
    downsample_methods = [DownSampleLanczosMethod(), DownSampleMeanMethod()]
    for size in vector_sizes:
        for downsample_method in downsample_methods:
            perform_benchmark_hierarchy_pp(images, labels[label_name], pp_predictions, name, query_name, number_of_samples,
                                           vector_size = size, downsample_method = downsample_method)
