...
def udf_pipeline(file_descriptor):
    ...
    file_descriptor.write(f"udf prediction loading time: {time.perf_counter() - st} (sec)\n")
    return results
...
def io_timer():
    timing = os.times()
    return (timing.system + timing.user) - timing.elapsed
...
class MyThread(threading.Thread):
    ...
    def Executemycode(self):
        ...
        import sys
        base_directory = '/nethome/jbang36/eva_jaeho/data/benchmark_results/speed_io'
        os.makedirs(base_directory, exist_ok=True)
        filename = 'results.txt'

        sys.stdout = open(os.path.join(base_directory, filename), 'w')
        print(profile_io_time(loader.load_images, video_directory))
