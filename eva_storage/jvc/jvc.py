...
class JVC:
    ...
    def preprocess(self, video_type, video_name, **kwargs):
        ...
        hierarchy_save_dir = os.path.join(self.base_directory, 'frame_hierarchy', video_type, video_name + '_jvc.npy')
    ...
    def decode(self, video_type, video_name, **kwargs):
        sample_count = kwargs.get('sample_count', 100) ## TODO: make sure the decoder takes care of edge cases
        video_directory = os.path.join( self.base_directory, video_type, video_name + '.mp4')
        hierarchy_directory = os.path.join( self.base_directory, 'frame_hierarchy', video_type, video_name + '.npy')
        extracted_images = self.decoder.run(video_directory, hierarchy_directory, number_of_samples = sample_count)
        return extracted_images


    def encode(self, jvc_video_name, **kwargs):
        save_directory = os.path.join( os.path.dirname(self.original_video_directory), jvc_video_name + '.mp4')
        self.encoder.run(self.images, self.hierarchy, self.original_video_directory, save_directory)
